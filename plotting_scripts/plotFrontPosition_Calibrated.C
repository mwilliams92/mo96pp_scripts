{
	...
	TFile *outfile = TFile::Open("FrontPositionCalib.root","RECREATE");
	outfile->cd();
	...
	for( int i=0; i<5; i++ )	{
		...
		for ( int j=0; j<4; j++ )	{
			...
			sprintf(draw,"SX3StripPositionCal>>hFrontPositionUpstream[%d][%d]", i, j);
			sprintf(cut,"SX3Upstream==1 && SX3Det==%d && SX3Strip==%d && SX3SectorEnergy>6600 && SX3SectorEnergy<7000", i, j);
			
			TCut Cut = cut;

			hFrontPositionUpstream[i][j] = new TH1D(hname, hname, 400, -2, 2);
	...
	cout << "Detector " << i << endl;
		...
		for ( int j=0; j<4; j++ )	{
			...
			sprintf(draw,"SX3StripPositionCal>>hFrontPositionDownstream[%d][%d]", i, j);
			sprintf(cut,"SX3Upstream==0 && SX3Det==%d && SX3Strip==%d && SX3SectorEnergy>6600 && SX3SectorEnergy<7000", i, j);
			
			TCut Cut = cut;

			hFrontPositionDownstream[i][j] = new TH1D(hname, hname, 400, -2, 2);
