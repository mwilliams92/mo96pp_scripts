...
fit_df = pd.read_csv('fit_gammas.csv')
fit_params = {}
...
for idx,row in fit_df.iterrows():
    for cen in projs.keys():
        ...
        if cen > row['Ex Min'] and cen < row['Ex Max']: 
            ...
            eff_cor_coinc = total_coinc/(eff_value * coinc_eff) # do any other corrections (LT? you want to here)
...
plt.plot(xlims, [1.0]*2, linestyle='dotted')

ax.set_ylim(ylims)
ax.set_xlim(xlims)

ax.set_xlabel('Excitation Energy (MeV)', fontsize = 16)
ax.set_ylabel('$\gamma$-ray Emission Probability', fontsize = 16)
